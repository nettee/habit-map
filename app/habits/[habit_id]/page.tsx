    ...
    <div className="bg-surface-main min-h-screen text-text-primary">
      <div className="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <main className="space-y-6">
          {/* 习惯信息卡片 */}
          <Card className="bg-white border-surface-divider shadow-sm">
            <CardHeader className="flex flex-row items-start gap-4">
              <div className="bg-brand-primary/10 p-3 rounded-full">
                <Target className="h-6 w-6 text-brand-primary" />
              </div>
              <div>
                <CardTitle className="text-2xl font-bold text-text-primary">{habitData.name}</CardTitle>
              </div>
            </CardHeader>
          </Card>
          ...
          <Card className="bg-white border-surface-divider shadow-sm">
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-text-primary">微行为</CardTitle>
                ...
                {habitData.microBehaviors.map((behavior, index) => (
                  <li key={index} className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <span className="text-text-secondary block">{behavior.action}</span>
                      <span className="text-sm text-text-secondary/70 mt-1 block">
          ...
          <Card className="bg-white border-surface-divider shadow-sm">
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-text-primary">最近7天触发情况</CardTitle>
              <CardDescription className="text-text-secondary">查看你最近一周的习惯完成情况。</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-7 gap-2">
                {habitData.recentTriggers.map((trigger, index) => {
                  const getBgColor = (count: number) => {
                    if (count === 0) return "bg-gray-200 text-text-secondary"
                    if (count === 1) return "bg-brand-primary/60 text-white" // 浅绿
                    if (count === 2) return "bg-brand-primary/80 text-white" // 中绿 (主绿色)
                    if (count >= 3) return "bg-brand-primary text-white" // 深绿
                    return "bg-gray-200 text-text-secondary"
                  }

                  return (
                    <div key={index} className="text-center">
                      <div className="text-xs text-text-secondary mb-2">{trigger.date}</div>
                      <div
